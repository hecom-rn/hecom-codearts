...
export async function initCommand(): Promise<void> {
  ...
  const answers = await inquirer.prompt([
    {
      ...
      message: 'IAM 认证端点:',
      default:
        existingConfig.HUAWEI_CLOUD_IAM_ENDPOINT || 'https://iam.cn-north-1.myhuaweicloud.com',
    },
    {
      type: 'input',
      name: 'region',
      message: '华为云区域:',
      default: existingConfig.HUAWEI_CLOUD_REGION || 'cn-north-1',
    },
    {
      type: 'input',
      name: 'username',
    ...
    },
    {
      ...
      default:
        existingConfig.CODEARTS_BASE_URL || 'https://projectman-ext.cn-north-1.myhuaweicloud.cn',
    },
    {
      type: 'input',
      name: 'projectId',
  ...
  try {
    ...
    console.log('\n您现在可以在任何目录使用以下命令：');
    console.log('  hecom-codearts daily              # 生成日报');
    console.log('  hecom-codearts work-hour          # 生成年度工时统计');
    console.log('\n提示：配置文件包含敏感信息，请妥善保管。');
  } catch (error) {
    console.error('\n❌ 保存配置文件失败:', error);
    process.exit(1);
